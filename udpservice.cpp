...
//    pthread_create(&thread,NULL,retransThreadFunc,(void*)rQueue);
    ...
    head.MESSAGE_ID   = message_id++;
    head.COMMAND_ID   = FILE_FINISH_REQ;
    head.SEQUENCE_ID  = sequence_id++;
    head.TOTAL_LENGTH = BUFSIZE;
    head.VERSION = 0;
    head.BLOCK_INDEX  = 0;
...
void *recvFileThread(void *arg)
{
    ...
    FileOperation *fOption = (FileOperation *)arg;
    //RetransQueue *rQueue= new RetransQueue;
    int sendNum = 0,fSize = fOption->getFileSize();
    fOption->setClientConnect();
    char *filebuf = new char[FILEBUF];
    memset(filebuf,0,FILEBUF);
    char *sendbuf = new char[BUFSIZE];
...
{
    int nfds = epoll_wait(m_epollfd,m_pEvents,m_iMaxFdNum,0);
    if(nfds < 0)
    {
...
void UDPService::fileReceived(MESSHEADER *cmd)
{

}

void UDPService::fileSendSuccessFul(MESSHEADER *cmd)
{
...
int UDPService::startRecvFile(MESSHEADER *pHead)
{
    ...
    if((&curNode->list) != head)
    {
        ...
        if(fileinfo.FILE_OPT->fileOpenToRecv(fName,curNode->FILE_BPOINT) < 0)
        {
...
void UDPService::getDatagram(MESSHEADER *cmd)
{
    FILE_PART_REG *ackMsg = (FILE_PART_REG *)(cmd + 1);

    char *buf = new char[FILEBUF];
    memcpy(buf,ackMsg + 1,strlen((char *)(ackMsg + 1)));
    struct list_head *entry = new list_head;
    FILEINFONODE *curNode = NULL;
    ...
    if(entry != recvHead)
    {
        if(curNode->FILE_OPT->sWindow->saveDatagram(buf,ackMsg->REG_FRAGMENT_NUM))
        {
            char *fileSeq = new char[MTU * 8],p = fileSeq;
            int len = 0;
            for(int i = 0; i < 8; i++)
            {
                len = strlen(m_pstrWindow + MTU * i);
                memcpy(p,m_pstrWindow + MTU * i,len);
                p = p + len;
            }
            curNode->FILE_OPT->writeFile(fileSeq,strlen(fileSeq));
            delete fileSeq;
        }else
        {
...
void UDPService::ackFileSegment(MESSHEADER *cmd)
{
